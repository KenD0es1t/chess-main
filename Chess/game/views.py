...
def chooseMode(request):
    if request.method == 'POST':
        # form should send a field 'mode' with values 'local' or 'ai'
        mode = request.POST.get('mode')
        if mode == 'local':
            request.session['aiColor'] = None
            return redirect('playLocal')
        if mode == 'ai':
            # expect a field 'mySelect' with 'white' or 'black'
...
def playAI(request):
    return render(request, 'chessPage.html', {'aiCol': request.session.get('aiColor')})
