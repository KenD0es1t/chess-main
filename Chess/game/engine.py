    ...
    if color:
        ...
        for move in moves:
            for newPlace in move[2]:
                oldPiece = board[int(newPlace[0])][int(newPlace[1])]
                oX = int(move[1][1])
                oY = int(move[1][0])
                ...
                n_board[oY][oX] = ''
                n_board[int(newPlace[0])][int(newPlace[1])] = move[0]


                eval = MiniMax(n_board, depth - 1, alpha, beta, not color)[0]

                n_board[int(newPlace[0])][int(newPlace[1])] = oldPiece
                n_board[oY][oX] = move[0]
...
def mover(oldPlace,newPlace,board,color):
    ...
    piece = board[oY][oX]
    board[oY][oX] = ''
    board[nY][nX] = promotionHandler(piece, color, nY)